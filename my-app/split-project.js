...
if (fs.existsSync(serverDir)) {
    ...
    serverFiles.forEach(file => {
        fs.renameSync(path.join(serverDir, file), path.join(backendDir, file));
    });
...
if (fs.existsSync(scriptsDir)) {
    fs.renameSync(scriptsDir, path.join(backendDir, 'scripts'));
}
...
if (fs.existsSync(envFile)) {
    ...
    fs.renameSync(envFile, path.join(frontendDir, '.env'));
}
